...
async def request_human_review(
    state: DiagramWorkflowState, review_type: str = "critique_review"
) -> Dict[str, Any]:
    ...
    if review_type == "critique_review":
        return {
            **base_data,
            ...
            "analysis": state.get("analysis").model_dump()
            if state.get("analysis")
            else None,
            "critique": state.get("critique").model_dump()
...
def get_review_instructions(review_type: str) -> str:
    ...
    return instructions.get(
        review_type, "Please review the workflow state and provide feedback."
    )


def process_human_feedback(
    state: DiagramWorkflowState, feedback: Dict[str, Any]
) -> Dict[str, Any]:
